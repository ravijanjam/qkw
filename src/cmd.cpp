    ...
    for (i = 0; i < argc; i++) {
      ...
      if (c == 1) {
        ...
        if (_azc == "V")
          sd->cq->print_format(_val, 1);
        if (_azc == "E")
          sd->cq->print_format(_val, 2);
      }

      if ((_azc == sd->scol) and
          (std::regex_search(_val.begin(), _val.end(), sd->rgx)) and (c == 0)) {
        c = 1;
        i = -1;
      }
...
void cmd::search(string &rgs, string col) {
  ...
  if (col == "L" or col == "V" or col == "E" or col == "all") {
    _qS = fmt::format(fmt::runtime(mq["Q14"]), this->tblname);
    sd->n = 3;
...
int cmd::writeToFile(string &fN, string opt, string dt) {
  ...
  if (opt == "L") {
    ...
    _qS = fmt::format(fmt::runtime(this->mqc["Q5"]), this->tblname, "L", dt);
    sd->n = 1;
    this->cproc(_qS, this->fn);
    sd->n = -1;
  }
...
void cmd::deletefromfile(string &fN, string opt) {
  ...
  if (opt == "L" || opt == "m+L")
    _vdata = this->fileparser(fN);
  if (opt == "id" || opt == "m+id")
    _vdata = this->fileparser(fN, "id");
  ...
  for (auto _data : _vdata) {
    ...
    if (not emptystr(_data[_str])) {
      ...
      if (opt == "L") {
        _qS = fmt::format(fmt::runtime(mqc["Q11"]), this->tblname, _key);
      }
      this->proc(_qS);
      sd->n = -1;
...
void cmd::insfromfile(string &fN, string opt) {
  ...
  for (auto _data : _vdata) {
    ...
    if (not emptystr(_data[_str])) {
      ...
      if (opt == "L")
        _qS = fmt::format(fmt::runtime(mqc["Q2.0"]), this->tblname, _mll[_str],
                          _data[_str]);
      if (opt == "m+L") {
        _qS = fmt::format(fmt::runtime(mqc["Q6.1"]), this->tblname, _mll[_str],
                          _data[_str], _mll["label"], _key);
      }
      this->cproc(_qS, this->fn);
      sd->n = -1;
    ...
    if (not emptystr(_data[_str])) {
      ...
      _qS = fmt::format(fmt::runtime(mqc["Q2.1"]), this->tblname, _mll[_str],
                        "NULL");
      this->cproc(_qS, this->fn);
      sd->n = -1;
    }
    ...
    if (not emptystr(_data[_str])) {
      ...
      _qS = fmt::format(fmt::runtime(mqc["Q6.2"]), this->tblname, _mll[_str],
                        _data[_str], _mll["label"], _key);
      this->cproc(_qS, this->fn);
      sd->n = -1;
...
int cmd::getexpl(string &dt, string sd) {
  ...
  string _qS;
  if (sd == "L") {
    _qS = fmt::format(fmt::runtime(this->mqc["Q5.1"]), this->tblname, "L", dt);
    ...
    _qS =
        fmt::format(fmt::runtime(this->mqc["Q5"]), this->tblname, "rowid", dt);
    this->sd->n = 2;
    this->proc(_qS);
    this->sd->n = -1;
  }
...
void cmd::getall() {
  ...
  string _qS;
  _qS = fmt::format(fmt::runtime(qkw::mq["Q14"]), this->tblname);
...
void cmd::setrLV(string &st) {
  ...
  qS = fmt::format(fmt::runtime(mqc["Q6"]), this->tblname, "V", _sm.str(2),
                   "rowid", _sm.str(1));
  sd->n = -1;
  this->proc(qS);
}
...
void cmd::createtbls(string &st) {
  ...
  if (_vs.size() > 0) {
    for (auto tn : _vs) {
      _qS = fmt::format(fmt::runtime(this->mqc["Q1"]), tn);
      this->proc(_qS);
      sd->n = -1;
    }
  } else {
    _qS = fmt::format(fmt::runtime(this->mqc["Q1"]), st);
    this->proc(_qS);
    sd->n = -1;
  }
}
...
int cmd::getV(string &st, string sd) {
  ...
  string _qS;
  if (sd == "V") {
    _qS = fmt::format(fmt::runtime(this->mqc["Q5"]), this->tblname, "L", st);
    ...
    _qS =
        fmt::format(fmt::runtime(this->mqc["Q5"]), this->tblname, "rowid", st);
    this->sd->n = 7;
    this->proc(_qS);
    this->sd->n = -1;
  }
...
void cmd::copytbl(string &from, string &to) {
  ...
  qS = fmt::format(fmt::runtime(this->mqc["Q1"]), to);
  this->proc(qS);
  qS = fmt::format(fmt::runtime(this->mqc["Q10"]), this->tblname, from, to);
  this->proc(qS);
}
