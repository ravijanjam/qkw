...
qkw::~qkw() { sqlite3_close(this->sdb); }
...
qkw::qkw(string &dbn) : utils() {
  ...
  sqlite3_create_function(this->sdb, "rxset", 3, SQLITE_UTF8, NULL, this->rxset,
                          NULL, NULL);
}
...
int qkw::initdb(char *dbn, int typ = 0) {
  ...
  qS = fmt::format(fmt::runtime(mq["Q5"]), this->tblname);
  sd->n = -1;
  this->proc(qS);
...
void qkw::search(string &rgs, string col) {
  ...
  if (col != "all") {
    sd->n = 6;
    qS = fmt::format(fmt::runtime(mq["Q1"]), this->tblname);
    this->proc(qS);
    sd->n = -1;
  } else {
    qS = fmt::format(fmt::runtime(mq["Q1"]), this->tblname);
...
int qkw::insertdata(string &sd) {
  ...
  qS = fmt::format(fmt::runtime(mq["Q7"]), this->tblname, "L,V", _st);
  this->proc(qS);
...
void qkw::mergetbls(string &tbl, string &tf) {
  ...
  if (_vs.size() > 1) {
    ...
    if (sd->ri == 0) {
      ...
      qS = fmt::format(fmt::runtime(mq["Q5.1"]), tf, sd->ss);
      proc(qS);

      qS = fmt::format(fmt::runtime(mq["Q28"]), tf);
      sd->n = 10;
      proc(qS);
      ...
      for (int i = 0; i < _vs.size() - 1; i++) {
        qS = fmt::format(fmt::runtime(mq["Q27"]), tf, sd->scol, _vs[i],
                         _vs[i + 1]);
        proc(qS);
      }
    }
  ...
  // else {
  //    fmt::format("\e[31mERROR: 2 tables required min, <T,T..,T>
  //    <new_tbl>\e[0m");
  //  }
}
...
int qkw::getschema(string &tbl) {
  ...
  qS = fmt::format(fmt::runtime(mq["Q26.1"]), tbl);
  sd->n = 9;
  this->proc(qS);
  sd->n = -1;
  return sd->ri;
...
void qkw::listtbls_v1(string tbl, string opt = "all") {
  ...
  if (opt == "all") {
    ...
    if (vtbls.size() > 0) {
      for (int i = 0; i < (vtbls.size()); i++) {
        ...
        qS = fmt::format(fmt::runtime(mq["Q26"]), _ss);
        sd->n = 8;
        this->proc(qS);
      }
  ...
  if (opt == "one") {
    ...
    sd->n = 8;
    qS = fmt::format(fmt::runtime(mq["Q26"]), tbl);
    this->proc(qS);
    sd->n = -1;
  }
}
...
void qkw::modifyvalue(string &lbl, string opt) {
  ...
  if (opt == "L") {
    ...
    this->sd->sm = _sm;
    qS = fmt::format(fmt::runtime(mq["Q9.2"]), this->tblname, _lf, _li);
    this->proc(qS);
    sd->n = -1;
  }
  if (opt == "r") {
    ...
    this->sd->sm = _sm;
    qS = fmt::format(fmt::runtime(mq["Q9.3"]), this->tblname, _lf, _li);
    this->proc(qS);
    sd->n = -1;
  }
...
void qkw::modifylabel(string &lbl, string opt) {
  ...
  if (opt == "L") {
    ...
    this->sd->sm = _sm;
    qS = fmt::format(fmt::runtime(mq["Q9"]), this->tblname, _lf, _li);
    this->proc(qS);
    sd->n = -1;
  }
  if (opt == "r") {
    ...
    this->sd->sm = _sm;
    qS = fmt::format(fmt::runtime(mq["Q9.1"]), this->tblname, _lf, _li);
    this->proc(qS);
    sd->n = -1;
  }
...
int qkw::editdata(string &rx, string &data, string opt) {
  ...
  if (opt == "L") {
    qS = fmt::format(fmt::runtime(mq["Q24"]), this->tblname, "L", rx, data);
    this->proc(qS);
  }
...
void qkw::gettblcontent(string &st, string &opt) {
  ...
  if (br) {
    qS = fmt::format(fmt::runtime(mq["Q17"]), this->tblname, opt, st);
    sd->n = 3;
  } else {
    qS = fmt::format(fmt::runtime(mq["Q15"]), this->tblname, opt, st);
    sd->n = 0;
  }
  this->proc(qS);
  sd->n = -1;
...
void qkw::getAlltblcontent(string &tbn) {
  ...
  if (br) {
    qS = fmt::format(fmt::runtime(mq["Q14"]), tbn);
    sd->n = 3;
  } else {
    qS = fmt::format(fmt::runtime(mq["Q1"]), tbn);
    sd->n = 0;
  }
...
void qkw::getlabel(string &sl, string opt) {
  ...
  sd->scol = "V";
  if (opt == "L") {
    qS = fmt::format(fmt::runtime(mq["Q23"]), this->tblname, "L", sl);
    sd->n = 7;
    this->proc(qS);
  }

  if (opt == "r") {
    qS = fmt::format(fmt::runtime(mq["Q23"]), this->tblname, "rowid", sl);
    sd->n = 7;
    this->proc(qS);
  }
  sd->n = -1;
...
void qkw::cleartbls(string &st) {
  ...
  for (auto _tbl : _vs) {
    qS = fmt::format(fmt::runtime(mq["Q8.0"]), _tbl);
    this->proc(qS);
  }
}
...
void qkw::delentries(string &inf, string type) {
  ...
  if (type == "L") {
    qS = fmt::format(fmt::runtime(mq["Q8"]), this->tblname, "L", inf);
    this->sd->n = -1;
    this->proc(qS);
  }

  if (type == "r") {
    qS = fmt::format(fmt::runtime(mq["Q8"]), this->tblname, "rowid", inf);
    this->sd->n = -1;
    this->proc(qS);
  }
}
...
void qkw::rxset(sqlite3_context *sqc, int argc, sqlite3_value **sqv) {
  ...
  if (argc == 3) {
    ...
    _sx = fmt::format(
        "{}", reinterpret_cast<const char *>(sqlite3_value_text(sqv[0])));
    _sp = fmt::format(
        "{}", reinterpret_cast<const char *>(sqlite3_value_text(sqv[1])));
    _sd = fmt::format(
        "{}", reinterpret_cast<const char *>(sqlite3_value_text(sqv[2])));
...
void qkw::deletetbls(string &tbls) {
  ...
  for (auto ts : vs) {
    ...
    cin >> opt;
    if (opt == "y") {
      ts = fmt::format(fmt::runtime(mq["Q4"]), ts);
      this->proc(ts);
    } else {
...
void qkw::copytbl(string &from, string &to) {
  ...
  qS = fmt::format(fmt::runtime(this->mq["Q1"]), to);
  this->proc(qS);

  qS = fmt::format(fmt::runtime(this->mq["Q13"]), from, to);
  this->proc(qS);
}
...
void qkw::proc(string &st) {
  ...
  int rc =
      sqlite3_exec(this->sdb, st.c_str(), this->fn, this->sd, &(this->zErrMsg));
  if (rc != SQLITE_OK) {
...
void qkw::createtbls(string &st) {
  ...
  for (auto tn : _vs) {
    _ss = fmt::format(fmt::runtime(this->mq["Q5"]), tn);
    this->proc(_ss);
    sd->n = -1;
  }
}
...
int qkw::renametbl(string &from, string &to) {
  ...
  if (f == 0) {
    string ss = fmt::format(fmt::runtime(mq["Q11"]), from, to);
    this->proc(ss);
    return 2;
  }
...
void qkw::listlabels(string &st, string opt) {
  ...
  if (opt == "LA") {
    qS = fmt::format(fmt::runtime(mq["Q29"]), this->tblname);
    sd->n = 11;
...
int qkw::fn(void *data, int argc, char **argv, char **azColName) {
  ...
  case 11: {
    ...
    for (int cc = sf->cc, i = 0, g1 = 1, g2 = 4, g3 = 25; i < argc;
         i++, sf->cc++) {
...
void qkw::tblcolcount(string &st) {
  ...
  this->sd->n = 1;
  st = fmt::format("\'{}\'", st);
  string sr = fmt::format(fmt::runtime(this->mq["Q10"]), st);
  this->proc(sr);
}
...
void qkw::query(string &sq) {
  ...
  cout << endl;
  this->sd->n = 0;
  string sr = fmt::format(fmt::runtime(this->mq["Q1"]), sq);
  this->proc(sr);
}
